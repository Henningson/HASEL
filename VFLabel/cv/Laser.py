...
class Laser:
    ...
    def readFromMAT(self, path):
        ...
        try:
            self.setLaserDimensions(
                f["lsr"]["lsrArrayDims"][0][0][0][0][0][0][0],
                f["lsr"]["lsrArrayDims"][0][0][0][0][1][0][0],
            )
        except:
        ...
        self._direction = np.matmul(
            -self._rotation_matrix, np.array([[0.0, 0.0, -1.0]]).T
        ).T
        ...
        for x in range(self._gridWidth):
            for y in range(self._gridHeight):
                laserField.append(
                    np.array(
                        [
                            np.tan((x - (self._gridWidth / 2.0)) * self._alpha),
                            np.tan((y - (self._gridHeight / 2.0)) * self._alpha),
                            -1.0,
                        ]
                    )
                )
