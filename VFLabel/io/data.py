...
def create_image_data(image_dir, video_file):
    ...
    os.system("ffmpeg -i {0} -start_number 0 {1}/%05d.png".format(video_file, image_dir))
    ...
    with open(path + ".rois") as file:
        ...
        for frame in tqdm(DICT['frames']):
            ...
            for point in frame['roi_positions']:
                ...
                cv2.circle(image, np.array([x, y]).astype(np.int), radius=6, thickness=-1, color=255)
...
if __name__ == "__main__":
    ...
    create_image_data(os.path.join(base_path, file_basename, "png"), "og_data/" + file_basename + ".mp4")
